...
router.post('/', authenticate, async (req, res) => {
  try {
    const adminId = req.user.adminId || req.user.uid || req.user.id;
    console.log('[COTIZ DEBUG] req.user:', JSON.stringify(req.user));
    console.log('[COTIZ DEBUG] adminId resuelto:', adminId);

    // Generar número atómicamente. El payload.numero (si viene) se descarta.
    const numero = await generarNumeroCotizacion(adminId);
    console.log('[COTIZ DEBUG] numero generado:', numero);

    // Eliminar numero del body (venga como sea: "", undefined, o string viejo)
    const { numero: _ignorado, ...resto } = req.body;

    const payload = {
      ...resto,
      numero,      // ← SIEMPRE el generado por backend (va después del spread)
      adminId,
    ...
    // Doble verificación: si por algún motivo numero quedó vacío, lanzar error
    if (!payload.numero) throw new Error('No se pudo generar el número de cotización');
